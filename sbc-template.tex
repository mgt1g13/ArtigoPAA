...
Assim, se somarmos o número de vezes que cada linha é executada, temos que a quantidade de instruções pode ser ser expressa pelo polinômio $f(n)$:
...
\begin{algorithm}[H]
  \caption{Força Bruta}\label{recursiveSubArray}
  \begin{algorithmic}[1]
...
A solução é descrita no Algoritmo~\ref{divideAndConquer}.
...
Existe ainda outra forma de resolver o problema do Subarray Máximo. Essa forma é descrita pelo Algoritmo ~\ref{recursiveSubArray}. Esse algortimo é baseado no famoso algoritmo de Kadane.

A ideia se baseia no fato de que, se temos uma soma acumulada negativa, podemos iniciar uma nova contagem no elemento corrente. Ou seja, começando no primeiro termo, vamos montando subarrays usando a ideia principal descrita anteriormente. Chamando recursivamente para o próximo elemento, calculamos o maior subarray terminando no próximo elemento. Avaliamos o retorno com o calculado e se retorna o maior.
